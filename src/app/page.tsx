type IconName = "dashboard" | "import" | "analysis" | "insights" | "reports" | "conversations" | "sentiment" | "critical" | "issue" | "menu";
...
function Sidebar() {
  return <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-slate-800 bg-slate-950 text-white lg:block">
    ...
    <nav aria-label="Main navigation" className="block px-4 pt-6">{navigation.map((item, index) => <a key={item.label} href={index === 0 ? "#overview" : `#${item.label.toLowerCase().replace(" ", "-")}`} className={`mb-1 flex shrink-0 items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-400 ${index === 0 ? "bg-indigo-500 text-white shadow-sm" : "text-slate-400 hover:bg-slate-900 hover:text-white"}`}><Icon name={item.icon} className="h-[18px] w-[18px]" />{item.label}</a>)}</nav>
...
function MobileHeader() {
  return <div className="relative z-20 flex h-16 items-center justify-between border-b border-slate-800 bg-slate-950 px-4 text-white lg:hidden">
    <div className="flex min-w-0 items-center gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-indigo-500 shadow-lg shadow-indigo-950/30"><span className="flex items-end gap-0.5" aria-hidden="true"><i className="h-2 w-0.5 rounded-full bg-white" /><i className="h-4 w-0.5 rounded-full bg-white" /><i className="h-3 w-0.5 rounded-full bg-white" /><i className="h-5 w-0.5 rounded-full bg-white" /><i className="h-2.5 w-0.5 rounded-full bg-white" /></span></div><div className="min-w-0"><p className="truncate text-sm font-semibold tracking-tight">Voice of Customer</p><p className="text-[10px] font-medium uppercase tracking-[0.18em] text-slate-400">AI Analyzer</p></div></div>
    <details className="group relative ml-3 shrink-0"><summary className="grid h-11 w-11 cursor-pointer list-none place-items-center rounded-xl border border-slate-700 text-slate-200 transition-colors hover:bg-slate-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-400 [&::-webkit-details-marker]:hidden"><span className="sr-only">Open navigation menu</span><Icon name="menu" className="h-5 w-5" /></summary><nav aria-label="Mobile navigation" className="absolute right-0 top-13 w-[min(19rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-slate-700 bg-slate-950 p-2 shadow-2xl shadow-slate-950/40">{navigation.map((item, index) => <a key={item.label} href={index === 0 ? "#overview" : `#${item.label.toLowerCase().replace(" ", "-")}`} className={`flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-indigo-400 ${index === 0 ? "bg-indigo-500 text-white" : "text-slate-300 hover:bg-slate-900 hover:text-white"}`}><Icon name={item.icon} className="h-[18px] w-[18px]" />{item.label}</a>)}</nav></details>
  </div>;
}
